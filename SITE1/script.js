...
const MobileMenu = (function () {
  ...
  const PANEL_MS = 380;
  const PANEL_EASE = 'cubic-bezier(0.2, 0.7, 0.2, 1)';
  let isOpen = false;
  let panelAnim = null;

  function inner() {
    return menu && menu.querySelector('.mobile-menu__inner');
  }
  ...
  function runPanelAnim(from, to, done) {
    const panel = inner();
    if (!panel) return;
    ...
    panelAnim = panel.animate(
      ...
      { duration: PANEL_MS, easing: PANEL_EASE, fill: 'forwards' }
    );
  ...
  function animateLinksIn() {
    if (prefersReducedMotion() || !menu) return;
    menu.querySelectorAll('.mobile-menu__nav a').forEach((link, i) => {
      link.animate(
        [
          { opacity: 0, transform: 'translateY(12px)' },
          { opacity: 1, transform: 'translateY(0)' },
        ],
        { duration: 350, easing: PANEL_EASE, fill: 'forwards', delay: 80 + i * 40 }
      );
    });
  }
  ...
  function resetLinks() {
    if (!menu) return;
    menu.querySelectorAll('.mobile-menu__nav a').forEach((link) => {
  ...
  function open() {
    ...
    requestAnimationFrame(() => {
      runPanelAnim('translateY(-100%)', 'translateY(0)', animateLinksIn);
    });
  }
  ...
  function close() {
    ...
    runPanelAnim('translateY(0)', 'translateY(-100%)', () => {
...
window.__jlCloseMobileMenu = () => MobileMenu.close();
window.__jlIsMobileMenuOpen = () => MobileMenu.isOpen();
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function (e) {
    ...
    if (targetElement) {
      ...
      // Close mobile menu if open (sem toggle — evita reabrir ao navegar)
      if (window.__jlIsMobileMenuOpen?.()) {
        window.__jlCloseMobileMenu?.();
      }
